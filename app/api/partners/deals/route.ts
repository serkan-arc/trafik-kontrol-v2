...
export async function GET(request: Request) {
  try {
    ...
    queryText += ` ORDER BY bd.created_at DESC`
...
export async function POST(request: Request) {
  try {
    ...
    const buyerCheck = await query(
      'SELECT buyer_code FROM buyers WHERE buyer_code = $1',
      [buyer_code]
    )
    ...
    // Check if active deal already exists for this buyer-offer combination
    const existingDeal = await query(
      'SELECT id FROM buyer_deals WHERE buyer_code = $1 AND offer_id = $2 AND status = $3',
      [buyer_code, offer_id, 'active']
    )
    
    if (existingDeal.rows.length > 0) {
      return NextResponse.json(
        { success: false, error: 'Active deal already exists for this buyer-offer combination' },
        { status: 400 }
      )
    }
